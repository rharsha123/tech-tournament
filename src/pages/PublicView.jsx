...
export default function PublicView({ onLogout }) {
  ...
  return (
    <div className="min-h-screen bg-slate-900 text-white font-sans">
      <header className="border-b border-slate-800 bg-slate-950 px-2 sm:px-3 py-1.5 shadow-xs">
        <div className="flex flex-wrap items-center gap-y-1.5 gap-x-2">
          {/* LOGO */}
          <div className="flex items-center space-x-1 shrink-0">
            <Trophy className="text-amber-400" size={13} />
            <span className="text-[10px] font-black tracking-wider text-slate-300 uppercase">Tech Tournaments</span>
          </div>

          {/* RIGHT-SIDE CONTROLS */}
          <div className="flex items-center gap-1.5 ml-auto order-2">
            <select
              value={selectedTournamentId}
              onChange={(e) => setSelectedTournamentId(e.target.value)}
              className="bg-emerald-600 text-white font-extrabold text-[10px] px-1.5 py-1 rounded max-w-[110px] sm:max-w-none focus:outline-none cursor-pointer truncate"
            >
              {tournaments.map(t => (
            ...
            <button 
              ...
              className="shrink-0 bg-slate-800 hover:bg-slate-700 text-amber-400 border border-slate-700 px-1.5 py-1 rounded text-[10px] font-extrabold flex items-center space-x-0.5"
              title="Media Gallery"
            >
              <Camera size={10} />
            ...
            <div className="flex items-center space-x-1 border-l border-slate-800 pl-1.5 shrink-0">
              {onLogout && (
                <button 
                  onClick={onLogout}
                  className="p-1 rounded bg-slate-800 hover:bg-red-600 text-slate-300 hover:text-white transition"
                  title="Logout"
                >
                  <LogOut size={13} />
                </button>
          ...
          {/* TABS SWITCHER */}
          <div className="flex items-center bg-slate-900 p-0.5 rounded border border-slate-800 w-full sm:w-auto order-3 overflow-x-auto">
            {[
              ...
              <button
                ...
                className={`flex-1 sm:flex-initial flex items-center justify-center space-x-1 px-2 py-1 rounded text-[10px] font-bold transition whitespace-nowrap ${
      ...
      <main className="p-3 sm:p-5 max-w-7xl mx-auto space-y-4">
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex items-center justify-between bg-slate-900 p-3 rounded-lg border border-slate-700 shadow-md">
                        ...
                        <span className="text-xs font-black text-slate-500 px-1.5 sm:px-3 uppercase tracking-widest shrink-0">VS</span>
        ...
        {publicTab === "bracket" && (
          <section className="bg-slate-800/60 border border-slate-700/80 rounded-xl p-3 sm:p-5 shadow-xl space-y-4 sm:space-y-5">
            <div className="flex flex-wrap items-center justify-between gap-2 border-b border-slate-700 pb-3">
              <h2 className="text-xs font-black tracking-wider uppercase flex items-center space-x-1 text-amber-400 min-w-0">
                <GitCommit size={15} className="shrink-0" />
                <span className="truncate">Tournament Hierarchy & Bracket ({activeTourn?.name})</span>
              </h2>
              <span className="text-[10px] bg-emerald-900 text-emerald-300 px-2.5 py-0.5 rounded font-bold shrink-0">{activeTourn?.sport}</span>
            </div>
        ...
        <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-xs z-50 flex items-center justify-center p-3">
          <div className="bg-slate-900 border border-slate-800 rounded-xl max-w-4xl w-full max-h-[85vh] flex flex-col shadow-2xl overflow-hidden">
            <div className="bg-slate-950 p-3 border-b border-slate-800 flex items-center justify-between gap-2">
              <h3 className="font-black text-xs text-amber-400 flex items-center space-x-1 min-w-0">
                <Camera size={14} className="shrink-0" />
                <span className="truncate">Tournament Media & Gallery ({activeTourn?.name})</span>
              </h3>
              <button onClick={() => setGalleryModalOpen(false)} className="text-slate-400 hover:text-white shrink-0"><X size={16}/></button>
            </div>
