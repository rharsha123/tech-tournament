...
export default function AdminDashboard() {
  ...
  const [fixtureData, setFixtureData] = useState({
    groupAId: "", teamA: "", groupBId: "", teamB: "", date: "", time: "", venue: "", videoUrl: ""
  });

  // Fixture Editing State
  const [editingMatchId, setEditingMatchId] = useState(null);
  const [editMatchData, setEditMatchData] = useState({ teamA: "", teamB: "", date: "", time: "", venue: "", videoUrl: "" });
  ...
  const handleCreateFixture = async (e) => {
    ...
    setFixtureData({ groupAId: "", teamA: "", groupBId: "", teamB: "", date: "", time: "", venue: "", videoUrl: "" });
  };
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden pb-16 md:pb-0">
        <header className="bg-white shadow-2xs border-b border-slate-200 px-5 py-2.5 flex flex-col sm:flex-row sm:items-center justify-between gap-2">
          <div>
            ...
            <p className="text-[10px] text-slate-500">Live Scoring & Media Management Console</p>
          </div>
        ...
        <main className="p-4 sm:p-5 flex-1 overflow-y-auto max-w-7xl w-full mx-auto space-y-5">
          ...
          {currentTournament?.photos && currentTournament.photos.length > 0 && (
            <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-2xs space-y-2">
              <h3 className="text-xs font-bold text-slate-800 uppercase">Tournament Photos ({currentTournament.photos.length})</h3>
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                {currentTournament.photos.map((photo, i) => (
                  <div key={i} className="relative group rounded-xl overflow-hidden border aspect-video bg-slate-100 shadow-xs">
                    <img src={photo} alt="Tournament" className="w-full h-full object-cover" />
                    <button 
                      onClick={() => handleDeleteTournamentPhoto(photo)}
          ...
          {activeTab === "groups_teams" && currentTournament && (
            <div className="space-y-4">
              ...
              {/* GROUPS, TEAMS, PLAYERS & PHOTOS TREE */}
                          ...
                          <div className="space-y-3 pl-2">
                            {group.teams.map(team => (
                              <div key={team.id} className="bg-white p-3 rounded-xl border border-slate-200 space-y-2">
                                ...
                                {/* Players List & Admin Player Photos */}
                                <div className="pl-3 border-l-2 border-slate-100 space-y-2 pt-1">
                                  ...
                                  {(!team.players || team.players.length === 0) ? (
                                    ...
                                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                                      {team.players.map(player => (
                                        <div key={player.id} className="bg-slate-50 p-2.5 rounded-lg border space-y-2">
                                          <div className="flex items-center justify-between">
                                          ...
                                          {/* Player Photos Display in Admin Screen */}
                                          {player.photos && player.photos.length > 0 && (
                                            <div className="grid grid-cols-3 gap-1.5 pt-1">
                                              {player.photos.map((ph, idx) => (
                                                <div key={idx} className="relative group rounded overflow-hidden aspect-video border bg-white">
                                                  <img src={ph} alt="Player" className="w-full h-full object-cover"/>
                                                  <button onClick={() => handleDeletePlayerPhoto(player.id, ph)} className="absolute top-0.5 right-0.5 bg-red-600 text-white p-0.5 rounded-full opacity-0 group-hover:opacity-100">
                                                    <Trash2 size={8}/>
                                                  </button>
                                                </div>
                                              ))}
          ...
          {activeTab === "fixtures" && currentTournament && (
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
              ...
              <div className="lg:col-span-2 bg-white p-4 rounded-2xl border border-slate-200 shadow-2xs">
                <h3 className="text-xs font-bold text-slate-800 mb-2.5">Matches & Scoring Console ({matches.filter(m => m.tournamentId === currentTournament.id).length})</h3>
                <div className="space-y-2.5">
                  {matches.filter((m) => m.tournamentId === currentTournament.id).map((match) => (
                      ...
                      {editingMatchId === match.id ? (
                        ...
                        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                          <div>
                            ...
                            {match.videoUrl && (
                              <span className="inline-flex items-center space-x-1 mt-1 text-[10px] bg-indigo-100 text-indigo-800 px-2 py-0.5 rounded-full font-bold">
                                <Video size={10} /> <span>Video URL Linked</span>
                              </span>
                            )}
                          </div>
                          <div className="flex items-center space-x-1.5 self-end sm:self-center">
...
function LiveScoringConsole({ match, sport, onClose }) {
  ...
  const handleBadmintonMinusPoint = async (side) => {
    ...
    await updateDoc(doc(db, "matches", match.id), {
      ...
      commentary: arrayUnion({
        timestamp: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }),
        text: `⚠️ Admin corrected score (-1 point to ${side === "A" ? match.teamA : match.teamB}). Current: ${scoresA[activeSetIdx]}-${scoresB[activeSetIdx]}`,
        type: "TEXT"
      })
    });
  };
