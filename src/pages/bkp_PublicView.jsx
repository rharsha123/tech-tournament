...
import { Trophy, Award, Activity, Radio, Camera, ImageIcon, X, Maximize2, Minimize2, Settings, LogOut } from "lucide-react";

export default function PublicView({ onNavigateAdmin, onLogout }) {
  const [tournaments, setTournaments] = useState([]);
  const [matches, setMatches] = useState([]);
  const [selectedTournamentId, setSelectedTournamentId] = useState("");
  const [publicTab, setPublicTab] = useState("matches"); // "matches" | "standings" | "gallery"
  ...
  return (
    <div className="min-h-screen bg-slate-900 text-white font-sans">
      {/* ULTRA-REDUCED 50% SMALLER MAIN HEADER BAR */}
      <header className="border-b border-slate-800 bg-slate-950 px-2 py-0.5 flex items-center justify-between gap-1 shadow-xs">
        ...
        {/* CONTROLS, SELECTOR & TINY ADMIN/LOGOUT ICONS */}
        <div className="flex items-center space-x-1">
          <button 
          ...
          {/* TINY ICON-ONLY ADMIN & LOGOUT BUTTONS */}
                ...
                {activeMatches.map((m) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {/* PROMINENT LARGE SCOREBOARD PANEL */}
